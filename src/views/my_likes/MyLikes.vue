...
<template>
  <div class="my_likes">
    ...
    <table>
      ...
      <tbody>
      <tr v-for="(item, index) in myLikes" :key="index" @click="clickItem(index)" :class="{item_checked:item.itemChecked}">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      clickItem(index){
        clickItem(this.myLikes, index, this.nowChecked)
      },
      toggleCollection(index){
        toggleCollection(this, this.myLikes, index)
      },
      toggleAttention(index){
        toggleAttention(this, this.myLikes, index)
      },
